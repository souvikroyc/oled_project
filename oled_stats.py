...
while True:
    ...
    with canvas(device) as draw:
        # Draw yellow text (top line)
        draw.text((0, 0), f"IP: {ip_address}", font=font, fill="yellow")
        
        # Combine CPU and Temperature on one line
        draw.text((0, 15), f"CPU: {cpu_usage}%  Temp: {temperature}", font=font, fill="blue")

        # Memory Usage in GB with percentage
        draw.text((0, 30), f"RAM: {memory_used:.2f}/{memory_total:.2f}GB ({memory_usage_percent}%)", font=font, fill="blue")
        
        # Disk Usage in GB with percentage
        draw.text((0, 45), f"Disk: {disk_used:.2f}/{disk_total:.2f}GB ({disk_usage_percent}%)", font=font, fill="blue")
